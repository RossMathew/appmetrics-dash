...
// metrics endpoint data - dash deletes its data after sending, so need to store our own
// just collect CPU and memory for the moment until we decide what data to send and in what groupings
var metricsCPUEvent = {
  process: 0,
  system: 0
}
...
exports.monitor = function(options) {
  ...
  var metricsEndpoint = function(req, res, next) {
    var agent = req.headers["user-agent"]
    if (agent.startsWith("Prometheus")) {
      res.write(
        "# HELP cpu Node.js application CPU usage data.\n" +
        "# TYPE cpu gauge\n" +
        "cpu_process " + metricsCPUEvent.process + "\n" +
        "cpu_system " + metricsCPUEvent.system + "\n" +
        "# HELP memory Node.js application memory usage data.\n" +
        "# TYPE memory gauge\n" +
        "memory_process_private " + metricsMemEvent.private + "\n" +
        "memory_process_physical " + metricsMemEvent.physical + "\n" +
        "memory_process_virtual " + metricsMemEvent.virtual + "\n" +
        "memory_system_used " + metricsMemEvent.physical_used + "\n" +
        "memory_system_total " + metricsMemEvent.physical_total + "\n");
      res.end();
    } else {
  ...
  if (!options.server) {
    ...
    app.use(url, site);
    app.use("metrics", metricsEndpont);
  ...
  monitoring.on('cpu', function(data) {
    latestCPUEvent = data;
    metricsCPUEvent = data
  });

  monitoring.on('memory', function(data) {
    latestMemEvent = data;
    metricsMemEvent = data;
  });
